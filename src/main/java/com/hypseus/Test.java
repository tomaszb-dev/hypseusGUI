...
*/
    ...
    @FXML
    private void saveTmpAtt(MouseEvent mouseEvent){
    ...
    private void setConfigAMPanelAIO() {
        ...
        Matcher sindenFound = checkSinden.matcher(options);
        Pattern checkjs = Pattern.compile("-sinden\\s+([1-9]|10)\\s+(x|w|r|g|b)");
        Matcher jsFound = checkSinden.matcher(options);
        if (sindenFound.find()) {
        ...
        if (options.contains("-force_aspect_ratio")) {
            forceAspect.setSelected(true);
        } else {
            forceAspect.setSelected(false);
        }
        if (options.contains("-scalefactor")) {
            AMscaleFactor.setSelected(true);
            AMscaleFactorSlider.setDisable(false);
            AMScaleFSliderView.setDisable(false);
            String scaleFac = options.substring(options.indexOf("-scalefactor"));
            int scaleFacInt = Integer.parseInt(scaleFac);
            AMscaleFactorSlider.setValue(scaleFacInt);
            AMScaleFSliderView.setText(Integer.toString(scaleFacInt));
        } else {
            AMscaleFactor.setSelected(false);
            AMscaleFactorSlider.setDisable(true);
            AMScaleFSliderView.setDisable(true);
        }
        if (options.contains("-scanline_alpha")) {
            AMScanLAlphaSilder.setDisable(false);
            AMScanASliderView.setDisable(false);
            String scanLine = options.substring(options.indexOf("-scanline_alpha"));
            int scanLineInt = Integer.parseInt(scanLine);
            AMScanLAlphaSilder.setValue(scanLineInt);
            AMScanASliderView.setText(Integer.toString(scanLineInt));
        } else {
...
*/
        ...
        SpinnerValueFactory<Integer> factory2 = new SpinnerValueFactory.IntegerSpinnerValueFactory(1, 10, 1);
        sindenFill1.setValueFactory(factory2);
